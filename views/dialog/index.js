...
function createDecision(left, top, width, height, id, record){
	...
	features[0].to_lines = [];
	features[0].style={fillColor:"#eeeeee", opacity: 1, strokeColor: "#339933", strokeOpacity: 1, strokeWidth: 3};
	features[0].attributes={id: id, "type" : "decisions", record : record};
	return features;
}
...
function updateState(){
	$.ajax( { type : "POST", url : "?pg=states&action=upd", cache : false, data: {"id":selected_feature.attributes.id, "name": $(".state_name input").val(), "description": $(".state_description input").val(), "info": $(".state_info input").val(), "document": $(".state_document input").val(), "video_link": $(".state_video_link input").val()}, success : function(d){}});
...
function updateDecision(){
	$.ajax( { type : "POST", url : "?pg=decisions&action=upd", cache : false, data: {"id":selected_feature.attributes.id, "name": $(".decision_name input").val(), "description": $(".decision_description input").val(), "question": $(".decision_question input").val(), "checked": ($(".decision_start:checked").length>0 ? "checked" : ""), "conditions":getConditionMatrix()}, success : function(d){}});
	if(selected_feature.attributes.record==undefined){
		selected_feature.attributes.record = {};
	}
